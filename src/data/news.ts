const NEWS = {
	status: "ok",
	totalResults: 37,
	articles: [
		{
			source: {
				id: null,
				name: "Yahoo Entertainment",
			},
			author: "Callie Lawson-Freeman",
			title:
				"NBA playoffs: Stephen Curry leads Warriors to Western Conference Semifinals with historic performance over Kings - Yahoo Sports",
			description:
				"The Kings had no answer for Curry's 50-point performance, a Game 7 record.",
			url: "https://sports.yahoo.com/nba-playoffs-stephen-curry-leads-warriors-to-western-conference-semifinals-with-historic-performance-over-kings-221726486.html",
			urlToImage:
				"https://s.yimg.com/ny/api/res/1.2/sWs6EbNvEDzZr_rZme1PgA--/YXBwaWQ9aGlnaGxhbmRlcjt3PTEyMDA7aD04MjQ-/https://s.yimg.com/os/creatr-uploaded-images/2023-04/0470af60-e7a4-11ed-bfae-48b22bb6a910",
			publishedAt: "2023-04-30T22:17:26Z",
			content:
				"It was win or go home on Sunday in Sacramento and Stephen Curry decided the Golden State Warriors' season wasnt ending yet. Advancing to the Western Conference semifinals, Curry made basketball magic… [+2091 chars]",
		},
		{
			source: {
				id: "fox-news",
				name: "Fox News",
			},
			author: "Janelle Ash, Larry Fink",
			title:
				"Willie Nelson at 90: Texas legend shares a birthday to remember with George Strait, Snoop Dogg, and others - Fox News",
			description:
				"Willie Nelson celebrated his 90th birthday with a special concert, filled with stars like Snoop Dogg, Lyle Lovett and Ethan Hawke.",
			url: "https://www.foxnews.com/entertainment/willie-nelson-texas-legend-birthday-remember-george-strait-snoop-dogg",
			urlToImage:
				"https://static.foxnews.com/foxnews.com/content/uploads/2023/04/1d278a8e-willie-nelson.jpg",
			publishedAt: "2023-04-30T21:15:00Z",
			content:
				'Texas troubadour Willie Nelson had a birthday party that will be remembered.\r\n"Long Story Short: Willie Nelson 90" kicked off on Saturday night, which was Nelson\'s birthday. The two-night event at th… [+4585 chars]',
		},
		{
			source: {
				id: null,
				name: "The Athletic",
			},
			author: "Mike Jones",
			title:
				"NFL Draft 2023 winners and losers: Eagles, Seahawks get stronger; Cowboys underwhelm - The Athletic",
			description:
				"Philadelphia stocked up on defense in the draft, while Seattle maximized its Russell Wilson trade assets. But not every team fared so well.",
			url: "https://theathletic.com/4472252/2023/04/29/nfl-draft-2023-winners-losers/",
			urlToImage:
				"https://cdn.theathletic.com/app/uploads/2023/04/29222105/GettyImages-1485906392-scaled-e1682821312234-1024x684.jpg",
			publishedAt: "2023-04-30T20:57:00Z",
			content:
				"The 2023 NFL Draft is officially in the books. With 259 selections made, the leagues teams will soon begin figuring out how to fit all of these puzzle pieces together.\r\nObviously, it will take some t… [+4416 chars]",
		},
		{
			source: {
				id: "reuters",
				name: "Reuters",
			},
			author: null,
			title: "Sudan sides trade blame, fight on despite ceasefire - Reuters",
			description:
				"Sudan's rival military forces accused each other of fresh violations of a ceasefire on Sunday as their deadly conflict rumbled on for a third week despite warnings of a slide towards civil war.",
			url: "https://www.reuters.com/world/africa/sudan-fighting-continues-ceasefire-set-end-midnight-2023-04-30/",
			urlToImage:
				"https://www.reuters.com/resizer/vgSeOPd4J-AlG4qjAx_9Ow_AcVI=/1200x628/smart/filters:quality(80)/cloudfront-us-east-2.images.arcpublishing.com/reuters/6FDTOSZPTFIPPFP4BBN72HPFXM.jpg",
			publishedAt: "2023-04-30T20:47:00Z",
			content:
				"KHARTOUM, April 30 (Reuters) - Sudan's rival military forces accused each other of fresh violations of a ceasefire on Sunday as their deadly conflict rumbled on for a third week despite warnings of a… [+5543 chars]",
		},
		{
			source: {
				id: "cnn",
				name: "CNN",
			},
			author: "Aaron Pellish",
			title:
				"Second US-led convoy evacuates private American citizens from Sudan conflict - CNN",
			description:
				"A second convoy of US citizens organized by the US government arrived in Port Sudan on Sunday as part of an effort to evacuate Americans from the Sudan conflict.",
			url: "https://www.cnn.com/2023/04/30/politics/us-convoy-sudan-conflict-evacuation/index.html",
			urlToImage:
				"https://media.cnn.com/api/v1/images/stellar/prod/230430160320-sudan-conflict-0430-restricted.jpg?c=16x9&q=w_800,c_fill",
			publishedAt: "2023-04-30T20:08:00Z",
			content:
				"A second convoy of US citizens organized by the US government arrived in Port Sudan on Sunday as part of an effort to evacuate Americans from the Sudan conflict.\r\nA second USG-organized convoy arrive… [+2681 chars]",
		},
		{
			source: {
				id: null,
				name: "CBS Sports",
			},
			author: "",
			title:
				"2023 NFL undrafted free agent tracker: Team-by-team list of prospect signings after NFL Draft - CBS Sports",
			description: "Here's a rundown of every undrafted rookie signing so far",
			url: "https://www.cbssports.com/nfl/draft/news/2023-nfl-undrafted-free-agent-tracker-team-by-team-list-of-prospect-signings-after-nfl-draft/",
			urlToImage:
				"https://sportshub.cbsistatic.com/i/r/2023/04/29/36fb60eb-f9e4-458f-91f1-7988eb9e2cad/thumbnail/1200x675/4bf664e29f5b4e2b8c9de46b2b78354d/adrian-martinez-g.jpg",
			publishedAt: "2023-04-30T19:18:00Z",
			content:
				"The 2023 NFL Draft has ended, with 259 players hearing their names called over the past three days. But for those players who didn't hear their names called, hope is not lost. Shortly after the draft… [+11576 chars]",
		},
		{
			source: {
				id: "usa-today",
				name: "USA Today",
			},
			author: "Ken Tran",
			title:
				"Rep. Alexandria Ocasio-Cortez not planning to run for Gillibrand's Senate seat in 2024: report - USA TODAY",
			description:
				"Democratic New York Congresswoman Alexandria Ocasio-Cortez is not planning to run a primary challenge for Kirsten Gillibrand's Senate seat in 2024.",
			url: "https://www.usatoday.com/story/news/politics/elections/2023/04/30/aoc-senate-2024-kirsten-gillibrand-primary/11772933002/",
			urlToImage:
				"https://www.gannett-cdn.com/presto/2023/04/28/USAT/da44bbf4-3ebd-4eda-896c-a0ffe79073c9-XXX_IMG_GTY_1246564047_A_HUM_1_1_5S12PD5NS.JPG?crop=1023,576,x0,y52&width=1023&height=576&format=pjpg&auto=webp",
			publishedAt: "2023-04-30T19:14:26Z",
			content:
				"Rep. Alexandria Ocasio-Cortez, D-N.Y., is not planning to issue a primary challenge for Sen. Kirsten Gillibrand, D-N.Y.s seat in the Senate, which is up for reelection in 2024.\r\nShe is not planning t… [+1027 chars]",
		},
		{
			source: {
				id: null,
				name: "New York Post",
			},
			author: "Isabel Keane",
			title:
				"Massive fire at Crimea oil reserve was opening blast of Ukraine's 'full-scale offensive' on Russia - New York Post ",
			description:
				"The massive fire that broke out at an oil reserve in Russian-occupied Sevastopol on Saturday was part of Ukraine’s “preparations” for its highly anticipated springtime counter-off…",
			url: "https://nypost.com/2023/04/30/ukraine-fire-at-oil-reserve-preparatory-for-counter-attack/",
			urlToImage:
				"https://nypost.com/wp-content/uploads/sites/2/2023/04/NYPICHPDPICT000010395788.jpg?quality=75&strip=all&w=1024",
			publishedAt: "2023-04-30T19:11:00Z",
			content:
				"The massive fire that broke out at an oil reserve in Russian-occupied Sevastopol on Saturday was an attack on Russian logistics and part of Ukraine’s “preparations” for a major counter-offensive.\r\nA … [+3682 chars]",
		},
		{
			source: {
				id: "fortune",
				name: "Fortune",
			},
			author: "Lance Lambert",
			title:
				"The housing market is shifting—just look at earnings published by 2 giant Fortune 500 homebuilders - Fortune",
			description:
				"A pair of homebuilders on the Fortune 500 list, No. 124 D.R. Horton and No. 267 PulteGroup, just reported a big drop in cancellation rates",
			url: "https://fortune.com/2023/04/30/housing-market-shifts-says-earnings-published-by-fortune-500-homebuilders-dr-horton-pultegroup/",
			urlToImage:
				"https://content.fortune.com/wp-content/uploads/2023/04/GettyImages-507136400-e1682881597408.jpg?resize=1200,600",
			publishedAt: "2023-04-30T19:05:00Z",
			content:
				"Federal Reserve actions to fight inflation through higher interest rates continued to impact homebuying demand in the [fourth] quarter, PulteGroup told investors in January.\r\nHowever, thats already o… [+3927 chars]",
		},
		{
			source: {
				id: "cnn",
				name: "CNN",
			},
			author: "Alli Rosenbloom",
			title:
				"'Super Mario Bros. Movie' set to become 10th animated film to cross $1 billion at the global box office - CNN",
			description:
				'Nearly one month after "The Super Mario Bros. Movie" premiered with an impressive opening weekend at the box office, the animated film is expected to hit another major milestone.',
			url: "https://www.cnn.com/2023/04/30/entertainment/super-mario-bros-movie-1-billion-box-office/index.html",
			urlToImage:
				"https://media.cnn.com/api/v1/images/stellar/prod/230430131519-04-super-mario-bros-movie.jpg?c=16x9&q=w_800,c_fill",
			publishedAt: "2023-04-30T18:53:00Z",
			content:
				"Nearly one month after The Super Mario Bros. Movie premiered with an impressive opening weekend at the box office, the animated film is expected to hit another major milestone. \r\nThe movie is set to … [+1507 chars]",
		},
		{
			source: {
				id: null,
				name: "Daily Mail",
			},
			author: "Katherine Lawton",
			title:
				"AI algorithm better at spotting cancerous nodules than other methods - Daily Mail",
			description:
				"The tool, designed by experts, can identify whether abnormal growths found on CT scans are cancerous.",
			url: "https://www.dailymail.co.uk/news/article-12031361/AI-accurately-spot-cancer-Algorithm-better-spotting-cancerous-nodules-methods.html",
			urlToImage:
				"https://i.dailymail.co.uk/1s/2023/04/30/19/70409013-0-image-a-19_1682879248496.jpg",
			publishedAt: "2023-04-30T18:38:35Z",
			content:
				"A new artificial intelligence tool can accurately identify cancer in a development doctors and scientists said could speed up diagnosis of the disease. \r\nThe algorithm performs more effectively than … [+2635 chars]",
		},
		{
			source: {
				id: null,
				name: "KTLA Los Angeles",
			},
			author: "Will Conybeare",
			title:
				"‘Pink eye’ among symptoms of new COVID-19 strain, health officials warn - KTLA Los Angeles",
			description:
				"The Los Angeles County Department of Public Health is warning residents that the newest Omicron strain of COVID-19 may come with some new symptoms.  The U.S. Centers for Disease Control and Prevention predicts the new variant, a strain called XBB.1.16 and kno…",
			url: "https://ktla.com/news/coronavirus/l-a-county-department-of-public-health-advising-residents-to-be-aware-of-new-covid-19-strain-symptoms/",
			urlToImage:
				"https://ktla.com/wp-content/uploads/sites/4/2023/04/AP23037634781772-e1676603444755.jpg?w=1280",
			publishedAt: "2023-04-30T18:37:00Z",
			content:
				"The Los Angeles County Department of Public Health is warning residents that the newest Omicron strain of COVID-19 may come with some new symptoms. \r\nThe U.S. Centers for Disease Control and Preventi… [+2139 chars]",
		},
		{
			source: {
				id: null,
				name: "YouTube",
			},
			author: null,
			title:
				"Tornado flips cars, damages homes in Florida - ABC7 News Bay Area",
			description: null,
			url: "https://www.youtube.com/watch?v=MegjFHTAE74",
			urlToImage: null,
			publishedAt: "2023-04-30T18:14:19Z",
			content:
				"Your browser isnt supported anymore. Update it to get the best YouTube experience and our latest features. Learn more\r\nRemind me later",
		},
		{
			source: {
				id: "the-washington-post",
				name: "The Washington Post",
			},
			author: "Tara Parker-Pope",
			title:
				"Michael J. Fox's Parkinson's has progressed; he doesn't expect to reach 80 - The Washington Post",
			description:
				"In a recent interview, Michael J. Fox described his ongoing battle with Parkinson's disease, which causes symptoms of uncontrollable jerks and slurred speech.",
			url: "https://www.washingtonpost.com/wellness/2023/04/30/michael-j-fox-parkinsons-disease-battle/",
			urlToImage:
				"https://www.washingtonpost.com/wp-apps/imrs.php?src=https://arc-anglerfish-washpost-prod-washpost.s3.amazonaws.com/public/QQFDOPAWRYDNJBV4ZAUAMUOJ4I.jpg&w=1440",
			publishedAt: "2023-04-30T18:01:34Z",
			content:
				"Comment on this story\r\nComment\r\nIn a revealing interview, the actor Michael J. Fox spoke about his own mortality and the challenges of living with Parkinsons disease for more than 30 years, including… [+3402 chars]",
		},
		{
			source: {
				id: null,
				name: "YouTube",
			},
			author: null,
			title:
				"What are the implications of Sudan's fighting on Egypt? | Inside Story - Al Jazeera English",
			description:
				"Sudan is in turmoil with no end in sight to the fighting between the army and the paramilitary Rapid Support Forces.Neighbouring nations, already dealing wit...",
			url: "https://www.youtube.com/watch?v=oTuqXu7NdB0",
			urlToImage: "https://i.ytimg.com/vi/oTuqXu7NdB0/maxresdefault.jpg",
			publishedAt: "2023-04-30T17:47:13Z",
			content: null,
		},
		{
			source: {
				id: null,
				name: "YouTube",
			},
			author: null,
			title:
				"Deal to take over First Republic Bank could come soon | GMA - ABC News",
			description:
				"Regulators are working through the weekend to try to find a buyer for First Republic bank.SUBSCRIBE to ABC News on YouTube: https://bit.ly/2vZb6yPLatest upda...",
			url: "https://www.youtube.com/watch?v=AoO4RfV8tM4",
			urlToImage: "https://i.ytimg.com/vi/AoO4RfV8tM4/maxresdefault.jpg",
			publishedAt: "2023-04-30T17:30:01Z",
			content: null,
		},
		{
			source: {
				id: "engadget",
				name: "Engadget",
			},
			author: "https://www.engadget.com/about/editors/igor-bonifacic",
			title:
				"Apple is reportedly redesigning watchOS around widgets - Engadget",
			description:
				"Apple is reportedly redesigning the Apple Watch’s user interface to make widgets a “central part” of how you will interact with the wearable..",
			url: "https://www.engadget.com/apple-is-reportedly-redesigning-watchos-around-widgets-162720331.html",
			urlToImage:
				"https://s.yimg.com/uu/api/res/1.2/t8TzdTrNCGrn4iSB45AH6w--~B/Zmk9ZmlsbDtoPTYzMDtweW9mZj0wO3c9MTIwMDthcHBpZD15dGFjaHlvbg--/https://media-mbst-pub-ue1.s3.amazonaws.com/creatr-uploaded-images/2022-09/42bfd920-3e6d-11ed-bfb6-ac78ec934026.cf.jpg",
			publishedAt: "2023-04-30T16:28:24Z",
			content:
				"Apple is reportedly working on its most significant software overhaul to watchOS in recent memory. According to Bloombergs\r\n Mark Gurman\r\n, the company is redesigning the Apple Watchs user interface … [+1469 chars]",
		},
		{
			source: {
				id: null,
				name: "MarketWatch",
			},
			author: "Vivien Lou Chen",
			title:
				"Stock-market investors want the Fed to answer this key question - MarketWatch",
			description:
				"Economy and markets are perched on a ‘knife’s-edge moment’ ahead of expected Fed rate hike",
			url: "https://www.marketwatch.com/story/stock-market-investors-want-an-answer-to-this-key-question-from-the-fed-f221e48d",
			urlToImage: "https://images.mktw.net/im-771341/social",
			publishedAt: "2023-04-30T16:01:00Z",
			content:
				"Multiple risks are raising the stakes in financial markets and for the U.S. economy as Federal Reserve policy makers prepare to gather this week.The Fed is widely expected to deliver a quarter-of-a-p… [+340 chars]",
		},
		{
			source: {
				id: null,
				name: "Forbes",
			},
			author: "Iain Martin",
			title:
				"The Kingdom Of Bhutan Has Been Quietly Mining Bitcoin For Years - Forbes",
			description:
				"The Himalayan kingdom confirmed it has been running a bitcoin mining operation as mystery surrounds the scale of its earlier cryptocurrency investments.",
			url: "https://www.forbes.com/sites/iainmartin/2023/04/30/bhutan-bitcoin-mining-crypto/",
			urlToImage:
				"https://imageio.forbes.com/specials-images/imageserve/644e397d741bc46ddf049831/0x0.jpg?format=jpg&width=1200",
			publishedAt: "2023-04-30T15:35:15Z",
			content:
				"The Himalayan kingdom confirmed it has quietly been running a bitcoin mining operation as mystery surrounds the scale of its earlier cryptocurrency investments. \r\nAFP via Getty Images\r\nThe Himalayan … [+12047 chars]",
		},
		{
			source: {
				id: null,
				name: "SciTechDaily",
			},
			author: null,
			title:
				"Mammalian Tree of Life Redefined: Genomic Time Machine Traces Back 100 Million Years of Evolution - SciTechDaily",
			description:
				"The research uses the genomes of 241 species and can be used to support animal and human health outcomes. Research led by a team of scientists from the Texas A&M School of Veterinary Medicine and Biomedical Sciences puts to bed the heated scientific debate re…",
			url: "https://scitechdaily.com/?p=273013",
			urlToImage:
				"https://scitechdaily.com/images/New-Research-Redefines-Mammalian-Tree-of-Life-scaled.jpg",
			publishedAt: "2023-04-30T15:29:16Z",
			content:
				"ByTexas A&amp;M UniversityApril 30, 2023\r\nResearchers from Texas A&amp;M University have used the largest mammalian genomic dataset to track the evolutionary history of mammals, concluding that mamma… [+11339 chars]",
		},
	],
};

export default NEWS;
